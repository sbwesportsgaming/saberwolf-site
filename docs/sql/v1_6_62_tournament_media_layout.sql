-- -SBW- v1.6.62
-- Capa/banner de torneios por upload + enquadramento persistido.
-- Reaproveita o bucket público já usado por perfis/equipes/organizadores.
...
drop policy if exists "sbw_tournament_assets_insert" on storage.objects;
create policy "sbw_tournament_assets_insert"
on storage.objects
for insert
to authenticated
with check (
  bucket_id = 'sbw-team-assets'
  and (storage.foldername(name))[1] = 'tournaments'
);

drop policy if exists "sbw_tournament_assets_update" on storage.objects;
create policy "sbw_tournament_assets_update"
on storage.objects
for update
to authenticated
using (
...
drop policy if exists "sbw_tournament_assets_delete" on storage.objects;
create policy "sbw_tournament_assets_delete"
on storage.objects
for delete
to authenticated
using (
...
begin
  ...
  current_metadata := coalesce(target_tournament.metadata, '{}'::jsonb);
  current_assets := coalesce(current_metadata->'tournamentAssets', '{}'::jsonb);
  incoming_assets := coalesce(
    payload->'tournamentAssets',
    payload->'tournament_assets',
    payload->'metadata'->'tournamentAssets',
    payload->'metadata'->'tournament_assets',
    '{}'::jsonb
  );
