-- =========================================================
-- v1.6.80.8 — Analytics: origem aproximada por IP temporário
-- =========================================================
-- Objetivo:
-- - Manter o IP fora do banco da -SBW-.
-- - Agregar país/estado/região salvos no metadata pela Edge Function.
-- - Separar Países, Estados e Regiões BR no Admin Master.
-- - Manter fallback por fuso/idioma para eventos antigos ou quando a Edge Function falhar.
-- =========================================================
...
begin
  ...
  geo_rows as (
    select
      nullif(coalesce(metadata->>'country_name', metadata->>'country', metadata->>'country_code'), '') as country_label,
      nullif(coalesce(metadata->>'country_code', metadata->>'geo_country_code'), '') as country_code,
      nullif(coalesce(metadata->>'state_code', metadata->>'region_code'), '') as state_code,
  ...
  countries as (
    ...
    from (
      select jsonb_build_object(
        'label', left(coalesce(country_label, country_code, 'Não identificado'), 80),
        'code', left(coalesce(country_code, ''), 12),
        'views', count(*)::integer
      ) as row
      from geo_rows
      where country_label is not null or country_code is not null
      group by coalesce(country_label, country_code, 'Não identificado'), coalesce(country_code, '')
      order by count(*) desc
      limit 8
    ) q
  ),
    ...
    from (
      select jsonb_build_object(
        'label', left(case
          ...
          when state_name is not null then concat_ws(' · ', state_name, country_label)
  ...
  location_labels as (
    select
      case
        when country_label is not null and state_name is not null then concat_ws(' · ', country_label, coalesce(state_code, state_name), brazil_region)
        when country_label is not null then country_label
        when timezone_label is not null then 'Fuso: ' || timezone_label
        when browser_language is not null then 'Idioma: ' || browser_language
        else 'Não informado'
      end as label
    from geo_rows
  ),
  ...
  from totals t, daily, pages, categories, devices, locations, countries, states, regions, pwa_split;
