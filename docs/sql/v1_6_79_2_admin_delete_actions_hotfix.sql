-- =========================================================
-- v1.6.79.2 — Hotfix Admin Master: exclusão segura de equipes/torneios
-- Plataforma -SBW-
-- =========================================================
-- Objetivo:
-- - reforçar as RPCs administrativas usadas pelos botões "Excluir do painel";
-- - preservar histórico/auditoria via metadata;
-- - manter exclusão como soft delete durante testes reais;
-- - ampliar a checagem de permissão Admin Master/Admin SBW para perfis e site_permissions.
-- =========================================================

-- ---------------------------------------------------------
-- Helper local: checagem administrativa mais tolerante
-- ---------------------------------------------------------
...
begin
  ...
  for row_json in
    ...
    where p.auth_user_id::text = current_auth_id
       ...
       or lower(coalesce(p.email, p.auth_email, '')) = current_email
  loop
...
begin
  ...
  if normalized_action = 'draft' then
    ...
    next_visibility := 'private';
    if previous_status in ('published', 'open', 'registration-open') then
      next_status := 'hidden';
    end if;
