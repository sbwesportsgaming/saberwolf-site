...
async function getOrganizerPermission(authUserId) {
  ...
  async function createBasicProfile(user) {
    ...
    const row = {
      // IMPORTANTE:
      // Não enviamos `id` aqui porque no Supabase a coluna public.profiles.id
      // é UUID. O banco deve gerar esse UUID automaticamente.
      // O identificador público do jogador fica em `username`/`slug`.
